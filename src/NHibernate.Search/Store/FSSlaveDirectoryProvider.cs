    ...
    public class FSSlaveDirectoryProvider : IDirectoryProvider
    {
        ...
        public void Start()
        {
            ...
            try
            {
                ...
                directory1 = FSDirectory.GetDirectory(subDir.FullName);
                if (create)
                {
                    log.DebugFormat("Initialize index: '{0}'", subDir.FullName);
                    IndexWriter iw1 = new IndexWriter(directory1, new StandardAnalyzer(LuceneVersion.Default), create, IndexWriter.MaxFieldLength.UNLIMITED);
                    iw1.Close();
                }

                subDir = new DirectoryInfo(Path.Combine(indexName, "2"));
                create = !IndexReader.IndexExists(subDir.FullName); 
                directory2 = FSDirectory.GetDirectory(subDir.FullName);
                if (create)
                {
                    log.DebugFormat("Initialize index: '{0}'", subDir.FullName);
										IndexWriter iw2 = new IndexWriter(directory2, new StandardAnalyzer(LuceneVersion.Default), create, IndexWriter.MaxFieldLength.UNLIMITED);
                    iw2.Close();
                }
